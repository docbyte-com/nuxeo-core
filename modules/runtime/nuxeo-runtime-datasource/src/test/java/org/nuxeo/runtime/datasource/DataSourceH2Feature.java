...
class DataSourceH2Feature extends AbstractDataSourceFeature {

    static {
        computeSystemProperty(DRIVER.withDefault(Driver.class.getName()), DEPRECATED_DRIVER);
        computeSystemProperty(
                URL.withDefault(String.format("jdbc:h2:mem:%s;DB_CLOSE_DELAY=-1;MODE=LEGACY", DATABASE_VALUE)),
                DEPRECATED_URL);
        computeSystemProperty(USER.withDefault("sa"), DEPRECATED_USER);
        computeSystemProperty(PASSWORD.withDefault(""), DEPRECATED_PASSWORD);
    }
