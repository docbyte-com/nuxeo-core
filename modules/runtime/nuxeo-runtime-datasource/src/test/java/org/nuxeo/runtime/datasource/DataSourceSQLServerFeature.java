...
class DataSourceSQLServerFeature extends AbstractDataSourceFeature {

    static {
        computeSystemProperty(DRIVER.withDefault(SQLServerDriver.class.getName()), DEPRECATED_DRIVER);
        String server = computeSystemProperty(SERVER, DEPRECATED_SERVER);
        String port = computeSystemProperty(PORT.withDefault("1433"), DEPRECATED_PORT);
        String user = computeSystemProperty(USER.withDefault("nuxeo"), DEPRECATED_USER);
        String password = computeSystemProperty(PASSWORD.withDefault("nuxeo"), DEPRECATED_PASSWORD);
        computeSystemProperty(URL.withDefault(
                String.format("jdbc:sqlserver://%s:%s;databaseName=%s;user=%s;password=%s;selectMethod=cursor", server,
                        port, DATABASE_VALUE, user, password)),
                DEPRECATED_URL);
    }
    ...
    protected void initializeDB() throws Exception {
        try (var connection = getConnection()) {
            doOnAllTables(connection, null, "DROP TABLE [%s]"); // no CASCADE...
            // sequences are supported since 11.x - not supported on Azure
            executeSql(connection,
                    "IF EXISTS (SELECT 1 FROM sys.sequences WHERE name = 'hierarchy_seq') DROP SEQUENCE hierarchy_seq");
        }
    }
