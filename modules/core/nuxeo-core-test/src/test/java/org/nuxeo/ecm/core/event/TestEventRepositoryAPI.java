...
public class TestEventRepositoryAPI {
    ...
    public void testDocumentRemove() {
        var doc = session.createDocumentModel("/", "file001", "File");
        ...
        try (CapturingEventListener listener = new CapturingEventListener()) {
            ...
            var aboutToRemoveEvent = listener.findFirstCapturedEventOrElseThrow(ABOUT_TO_REMOVE);
            var eventCtx = (DocumentEventContext) aboutToRemoveEvent.getContext();
            assertEquals("file001", eventCtx.getProperty("docTitle"));
            assertEquals(docId, eventCtx.getSourceDocument().getId());

            var documentRemoved = listener.findFirstCapturedEventOrElseThrow(DOCUMENT_REMOVED);
    ...
    public void testVersionRemove() {
        ...
        var versionRef = doc.checkIn(VersioningOption.MINOR, "a comment");
        ...
        try (CapturingEventListener listener = new CapturingEventListener()) {
            ...
            var aboutToRemoveVersionEvent = listener.findFirstCapturedEventOrElseThrow(ABOUT_TO_REMOVE_VERSION);
            var eventCtx = (DocumentEventContext) aboutToRemoveVersionEvent.getContext();
            assertEquals("file001", eventCtx.getProperty("docTitle"));
            assertEquals(docId, eventCtx.getProperty("docSource"));
            assertEquals(versionId, eventCtx.getSourceDocument().getId());

            var versionRemoved = listener.findFirstCapturedEventOrElseThrow(VERSION_REMOVED);
    ...
    public void testProxyRemove() {
        ...
        var proxy = session.createProxy(doc.getRef(), new PathRef("/folder"));
        ...
        try (CapturingEventListener listener = new CapturingEventListener()) {
            ...
            var proxyRemoved = listener.findFirstCapturedEventOrElseThrow(PROXY_REMOVED);
    ...
    public void testProxyOnVersionRemove() {
        ...
        var proxy = session.createProxy(versionRef, new PathRef("/folder"));
        String proxyId = proxy.getId();
        txFeature.nextTransaction();
