...
public class TestColdStorageBlobStore {
    ...
    public void testBlobStoreStatus() throws IOException, InterruptedException {
        ...
        await().pollDelay(DummyBlobProvider.RESTORE_DELAY_MILLISECONDS, TimeUnit.MILLISECONDS)
               .atMost(2 * DummyBlobProvider.RESTORE_DELAY_MILLISECONDS, TimeUnit.MILLISECONDS)
               .until(() -> {
                   var blobStatus = getBlobStatus();
                   return blobStatus.isDownloadable() && !blobStatus.isOngoingRestore();
               });
    }
