 ...
 * @since 6.0
 */
public final class AggregateDateRangeParser {

    public static DateRangeAggregationBuilder getAggregate(Aggregate<? extends Bucket> agg) {
        DateRangeAggregationBuilder ret = AggregationBuilders.dateRange(agg.getId())
                                                             .field(getFieldName(agg.getField()));
        for (AggregateRangeDateDefinition range : agg.getDateRanges()) {
    ...
    public static QueryBuilder getFilter(AggregateDateRange agg) {
    ...
    public static List<BucketRangeDate> parseBuckets(Collection<? extends MultiBucketsAggregation.Bucket> buckets) {
        List<BucketRangeDate> ret = new ArrayList<>(buckets.size());
        for (MultiBucketsAggregation.Bucket bucket : buckets) {
            Range.Bucket rangeBucket = (Range.Bucket) bucket;
            ZonedDateTime fromZDT = (ZonedDateTime) rangeBucket.getFrom();
            ZonedDateTime toZDT = (ZonedDateTime) rangeBucket.getTo();
            ret.add(new BucketRangeDate(bucket.getKeyAsString(), fromZDT, toZDT, rangeBucket.getDocCount()));
        }
        return ret;
    }
