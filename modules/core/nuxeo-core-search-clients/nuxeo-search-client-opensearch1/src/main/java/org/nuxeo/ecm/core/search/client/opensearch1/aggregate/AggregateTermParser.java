...
import static org.nuxeo.ecm.platform.query.api.AggregateConstants.AGG_ORDER_TERM_ASC;
import static org.nuxeo.ecm.platform.query.api.AggregateConstants.AGG_ORDER_TERM_DESC;
 ...
 * @since 6.0
 */
public final class AggregateTermParser {

    private AggregateTermParser() {
    }

    public static TermsAggregationBuilder getAggregate(Aggregate<? extends Bucket> agg) {
        TermsAggregationBuilder ret = AggregationBuilders.terms(agg.getId()).field(getFieldName(agg.getField()));
