...
public class TestSetPropertiesActionOptimizations {
    ...
    public void testDisableAudit() {
        ...
        var queryBuilder = new AuditQueryBuilder().predicate(Predicates.eq(LOG_DOC_UUID, note.getId()))
                                                  .and(Predicates.eq(LOG_REPOSITORY_ID, session.getRepositoryName()))
                                                  .defaultOrder();
        int initialLogEntries = backend.queryLogs(queryBuilder).size();

        bulkService.submit(createBuilder().build());
        transactions.nextTransaction();
        // documentCheckin and documentModified
        assertEquals(initialLogEntries + 2, backend.queryLogs(queryBuilder).size());

        bulkService.submit(createBuilder().param(DISABLE_AUDIT_LOGGER, TRUE).build());
        transactions.nextTransaction();
        // only documentCheckin
        assertEquals(initialLogEntries + 3, backend.queryLogs(queryBuilder).size());

        bulkService.submit(createBuilder().param(PARAM_VERSIONING_OPTION, NONE.toString()).build());
        transactions.nextTransaction();
        // only documentModified
        assertEquals(initialLogEntries + 4, backend.queryLogs(queryBuilder).size());
    ...
    // NXP-30700
    @Test
    @Deploy("org.nuxeo.audit.test.test:OSGI-INF/test-bulk-disable-automatic-versioning-contrib.xml")
    public void testDisableAutomaticVersioning() {
