...
public class BinaryMetadataSyncListener implements EventListener, Synchronization {
    ...
    public void handleEvent(Event event) {
        ...
        var binaryMetadataService = Framework.getService(BinaryMetadataService.class);
        var metadataUpdates = binaryMetadataService.getMetadataUpdates(doc, creation)
                                                   .stream()
                                                   .collect(partitioningBy(MetadataMappingUpdate::isAsync));
        ...
        if (DOCUMENT_CREATED.equals(eventName) || BEFORE_DOC_UPDATE.equals(eventName)) {
            var asyncUpdates = metadataUpdates.get(TRUE);
