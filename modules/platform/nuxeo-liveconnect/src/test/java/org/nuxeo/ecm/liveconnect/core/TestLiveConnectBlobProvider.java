...
import static org.nuxeo.ecm.liveconnect.LiveConnectFeature.createBlobInfo;
...
@Deploy("org.nuxeo.ecm.liveconnect.test:OSGI-INF/test-core-cache-config.xml")
@Deploy("org.nuxeo.ecm.liveconnect.test:OSGI-INF/test-core-config.xml")
@Deploy("org.nuxeo.ecm.liveconnect.test:OSGI-INF/test-core-pageprovider-contrib.xml")
public class TestLiveConnectBlobProvider {
    ...
    protected static final byte[] FILE_1_BYTES = "picture of a tiger".getBytes(UTF_8);
    ...
    @Before
    public void before() {
        blobProvider = (MockLiveConnectBlobProvider) blobManager.getBlobProvider(SERVICE_CORE_ID);
        assertNotNull(blobProvider);
    }

    @After
    public void after() {
        Framework.getService(CacheService.class).getCache(SERVICE_CORE_ID).invalidateAll();
    }
    ...
    public void testReadBlob() throws Exception {
        BlobInfo blobInfo = createBlobInfo(SERVICE_CORE_ID, FILE_1_ID);
    ...
    public void testReadBrokenBlob() throws Exception {
        ...
        BlobInfo blobInfo = createBlobInfo(SERVICE_CORE_ID, INVALID_FILE_ID);
        Blob blob = blobProvider.readBlob(blobInfo);
    ...
    public void testWriteBlob() throws Exception {
        blobProvider.writeBlob(createBlob(SERVICE_CORE_ID, FILE_1_ID));
    }

    @Test
    public void testCheckChangesAndUpdateBlobWithUpdate() {
        DocumentModel doc = session.createDocumentModel("parent", "file-1", "File");
        doc.setPropertyValue("content", createBlob(SERVICE_CORE_ID, FILE_1_ID, ""));
    ...
    public void testCheckChangesAndUpdateBlobWithoutUpdate() {
        ...
        doc = session.createDocumentModel("parent", "file-1", "File");
        doc.setPropertyValue("content", createBlob(SERVICE_CORE_ID, FILE_1_ID, FILE_1_DIGEST));
        docs = blobProvider.checkChangesAndUpdateBlob(Collections.singletonList(doc));
        assertTrue(docs.isEmpty());

        doc = session.createDocumentModel("parent", "file-1", "File");
        doc.setPropertyValue("content",
                createBlob(SERVICE_CORE_ID, FILE_1_ID, FILE_1_DIGEST, UUID.randomUUID().toString()));
        docs = blobProvider.checkChangesAndUpdateBlob(Collections.singletonList(doc));
        assertTrue(docs.isEmpty());
    }
    ...
    public void testToBlobWithFile() {
        LiveConnectFile file = new MockLiveConnectFile(new LiveConnectFileInfo(USER_ID, FILE_1_ID), FILE_1_NAME,
                FILE_1_SIZE, FILE_1_DIGEST);
        SimpleManagedBlob blob = blobProvider.toBlob(file);
        assertEquals(SERVICE_CORE_ID + ':' + USER_ID + ':' + FILE_1_ID, blob.getKey());
        assertEquals(FILE_1_NAME, blob.getFilename());
        assertEquals(FILE_1_SIZE, blob.getLength());
    ...
    public void testToBlobWithFileWithRevision() {
        ...
        LiveConnectFile file = new MockLiveConnectFile(new LiveConnectFileInfo(USER_ID, FILE_1_ID, revision),
                FILE_1_NAME, FILE_1_SIZE, FILE_1_DIGEST);
        SimpleManagedBlob blob = blobProvider.toBlob(file);
        assertEquals(SERVICE_CORE_ID + ':' + USER_ID + ':' + FILE_1_ID + ':' + revision, blob.getKey());
        assertEquals(FILE_1_NAME, blob.getFilename());
        assertEquals(FILE_1_SIZE, blob.getLength());
        assertEquals(FILE_1_DIGEST, blob.getDigest());
    }
    ...
    public void testToBlobWithFileInfo() throws Exception {
        LiveConnectFileInfo fileInfo = new LiveConnectFileInfo(USER_ID, FILE_1_ID);
    ...
    public void testToFileInfo() {
        SimpleManagedBlob blob = createBlob(SERVICE_CORE_ID, FILE_1_ID);
        LiveConnectFileInfo fileInfo = blobProvider.toFileInfo(blob);
        assertEquals(USER_ID, fileInfo.getUser());
    ...
    public void testToFileInfoWithRevision() {
        String revision = UUID.randomUUID().toString();
        BlobInfo blobInfo = createBlobInfo(SERVICE_CORE_ID, FILE_1_ID, FILE_1_DIGEST, revision);
    ...
    public void testFileCache() {
        ...
        LiveConnectFileInfo fileInfo = new LiveConnectFileInfo(USER_ID, id);
    ...
    private DocumentModel createDocumentWithBlob(long id, String digest) {
        ...
        testFile.setPropertyValue("file:content", createBlob(SERVICE_CORE_ID, FILE_1_ID, digest));
