...
import static org.nuxeo.ecm.webengine.app.HtmlNuxeoExceptionWriter.TYPE_TEXT_HTML;
...
@Singleton
@Provider
@Produces(TYPE_TEXT_HTML)
public class HtmlNuxeoExceptionWriter implements MessageBodyWriter<NuxeoException> {

    // set the quality to 0.1 in order to let JsonNuxeoExceptionWriter be the preferred one when no Accept is given
    // this has effect during MediaType resolution, see ContentTypeDeterminer#determineResponseMediaType
    protected static final String TYPE_TEXT_HTML = MediaType.TEXT_HTML + ';' + Quality.QUALITY_SOURCE_PARAMETER_NAME
            + "=0.1";
    ...
    public boolean isWriteable(Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType) {
        return NuxeoException.class.isAssignableFrom(type)
                // mediaType is null when exception happened during route resolution and mediaType hasn't been resolved
                && mediaType != null;
    }
