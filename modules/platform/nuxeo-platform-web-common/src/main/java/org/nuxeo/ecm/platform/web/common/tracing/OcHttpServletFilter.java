...
/**
 * This filter class implements Filter interface called by web container. The filter is used as an interceptor to enable
...
public class OcHttpServletFilter implements Filter {
    ...
    /**
     * Set optional OC_TRACE_PROPAGATOR attribute in {@link ServletContext} with {@link TextFormat} propagator. By
     * default {@code TraceContextFormat} is used to propagate trace context.
     *
     * @since 0.20
     */
    ...
    /**
     * Set optional OC_EXTRACTOR attribute in {@link ServletContext} with {@link HttpExtractor} customExtractor. Default
     * extractor is used if custom extractor is not provided.
     *
     * @since 0.20
     */
    ...
    /**
     * Set optional OC_PUBLIC_ENDPOINT attribute in {@link ServletContext} with {@link Boolean} publicEndpoint. set to
     * true for publicly accessible HTTP(S) server. If true then incoming * tracecontext will be added as a link instead
     * of as a parent. By default it is set to true.
     *
     * @since 0.20
     */
    ...
    @VisibleForTesting
    static final Getter<HttpServletRequest> getter = new Getter<HttpServletRequest>() {
        @Nullable
        @Override
    ...
    static HttpServerHandler<HttpServletRequest, HttpServletResponse, HttpServletRequest> buildHttpServerHandler() {
        return buildHttpServerHandlerWithOptions(new OcHttpServletExtractor(),
                Tracing.getPropagationComponent().getTraceContextFormat(), /* publicEndpoint= */ false);
    }

    static HttpServerHandler<HttpServletRequest, HttpServletResponse, HttpServletRequest> buildHttpServerHandlerWithOptions(
            HttpExtractor<HttpServletRequest, HttpServletResponse> extractor, TextFormat propagator,
            Boolean publicEndpoint) {
        return new HttpServerHandler<HttpServletRequest, HttpServletResponse, HttpServletRequest>(Tracing.getTracer(),
