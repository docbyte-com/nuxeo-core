...
public class OcHttpServletUtil {
    ...
    static void recordMessageSentEvent(
            ...
            HttpRequestContext context, HttpServletResponse response) {
        if (response != null) {
    ...
    /**
     * Enters the scope of code where the given {@link ServletRequest} will be processed and returns an object that
     * represents the scope. The scope is exited when the returned object is closed. A span created for the
     * {@link ServletRequest} is set to the current Context.
     * <p>
     ...
     * @return an object that defines a scope where the span associated with the given {@link ServletRequest} will be
