...
package org.nuxeo.ecm.restapi.server.routing;
...
public class WorkflowModelObject extends DefaultObject {
    ...
    public List<DocumentRoute> getWorkflowModels(@Context UriInfo uriInfo) {
        return Framework.getService(DocumentRoutingService.class)
                        .getAvailableDocumentRouteModel(getContext().getCoreSession());
    }

    @GET
    @Path("{modelName}")
    public DocumentRoute getWorkflowModel(@PathParam("modelName") String modelName) {
        DocumentRoute result = Framework.getService(DocumentRoutingService.class)
                                        .getRouteModelWithId(getContext().getCoreSession(), modelName);
        return result;
    }
    ...
    public JsonGraphRoute getWorkflowModelGraph(@PathParam("modelName") String modelName) {
        try {
            final String id = Framework.getService(DocumentRoutingService.class)
                                       .getRouteModelDocIdWithId(getContext().getCoreSession(), modelName);
