nuxeo-rest-opensearch-passthrough
==================================

## About

This addon exposes a limited set of Read Only OpenSearch HTTP REST API, taking in account the Nuxeo authentication
and authorization.

Before Nuxeo LTS 2025 this module was named `nuxeo-elasticsearch-http-read-only`.

To keep backward compatibility the API is still exposed as `/es/`.
...
- An OpenSearch instance with an HTTP REST API, the default port is 9200.
- Deploy the `nuxeo-rest-opensearch1-passthrough-VERSION.jar` into your Nuxeo server under `nxserver/bundles/`.
...
- Limit access to index (and types) defined inside Nuxeo.

For instance if your REST client application want to query like this:
...
Nuxeo will submit only HTTP GET request to OpenSearch, even if Nuxeo accepts search using HTTP POST.

The Document GET API is also filtered, for non Administrator user, there is a first request to retrieve the document ACL
