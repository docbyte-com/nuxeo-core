...
Extracted licenses:
- mvn -nsu license:aggregate-add-third-party -Pdistrib
  - target/generated-sources/license/THIRD-PARTY.txt
Extracted dependencies:
- mvn -nsu dependency:tree -Pdistrib -DoutputFile=\\${session.executionRootDirectory}/target/generated-sources/dependency-tree.log -DappendOutput=true
...
def parse_licenses(filename):
    ...
    with open(filename) as f:
        for line in f:
            ...
            versions[ga] = version
            descriptions[ga] = '[' + name + '](' + url + ')'
            ARTIFACT_GROUP[artifact] = group
    return licenses, versions, descriptions

def fix_unknown(licenses, add_licenses, versions, add_versions):
    for ga, license_list in licenses.items():
...
def dump(licenses, versions, descriptions, gas, whitelist):
    ...
    for ga, galist in licenses.items():
        if ga in gas:
            gas.remove(ga)
            out = blacks
...
def format_line(ga, licences, versions, descriptions):
    ...
    return '| ' + artifact + '-' + version + '.jar' + ' | ' + info + ' | ' + version + ' | ' + ', '.join(galist) + ' |'

def main():
    chdir_scripts()
    licenses, versions, descriptions = parse_licenses(TP)
    add_licenses, add_versions, _ = parse_licenses(UNKNOWN)
